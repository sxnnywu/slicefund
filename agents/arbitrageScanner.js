...
const SYSTEM_PROMPT =
  "You are an arbitrage validation agent for Slidefund. You receive raw arbitrage alerts containing two prediction market positions on the same underlying question but priced differently across platforms. Your job is to validate whether this is a genuine arbitrage opportunity by checking: is the spread large enough after fees and slippage to be worth trading, is this a duplicate of a recently seen alert, and is there any obvious reason the price discrepancy is legitimate rather than exploitable. Return a structured response with: decision (CONFIRMED or REJECTED), the spread value, your reasoning, and a confidence score from 0-1.";
const TEST_MESSAGE =
  "Raw alert: Kalshi YES @ 0.42, Polymarket YES @ 0.58, question: Will the Fed cut rates in Q1 2025";
...
export async function saveIds(assistantId, threadId) {
  try {
    ...
    const appended = `${needsLeadingNewline ? "\n" : ""}ARB_SCANNER_ASSISTANT_ID=${assistantId}\nARB_SCANNER_THREAD_ID=${threadId}\n`;
